...
public interface SubmissionRepository extends CrudRepository<Submission, Integer> {
    ...
    @Query(value = "SELECT * FROM submissions WHERE player_id = ?1 AND challenge_id = ?2 AND id != ?3 AND state = 'APPROVED' AND type = 'CHALLENGE_SPEED' ORDER BY id DESC LIMIT 1", nativeQuery = true)
    Optional<Submission> getPreviousSpeedrunChallengeSubmission(int playerId, int challengeId, int currentSubmissionId);
    
    @Query(value = "SELECT * FROM submissions WHERE player_id = ?1 AND challenge_id = ?2 AND challenge_relay_component_id = ?3 AND id != ?4 AND state = 'APPROVED' AND type = 'CHALLENGE_SPEED' ORDER BY id DESC LIMIT 1", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM submissions WHERE player_id = ?1 AND challenge_id = ?2 AND id != ?3 AND state = 'APPROVED' AND type = 'CHALLENGE_POINTS' ORDER BY id DESC LIMIT 1", nativeQuery = true)
    Optional<Submission> getPreviousPointsChallengeSubmission(int playerId, int challengeId, int currentSubmissionId);
